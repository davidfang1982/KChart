...
define(
    function (require) {
        ...
        var Storage = function () {
            // 所有常规形状，id索引的map，所有的shape和group
            this._elements = {};

            // 高亮层形状，不稳定，动态增删，数组位置也是z轴方向，靠前显示在下方
            this._hoverElements = [];

            this._roots = [];//列表 所有的shape和group
         ...
         */
        Storage.prototype.updateShapeList = function () {//updateShapeList时才会加入到_shapeList中
            this._shapeListOffset = 0;
            for (var i = 0, len = this._roots.length; i < len; i++) {
                var root = this._roots[i];
                this._updateAndAddShape(root);//会添加到_shapeList中
            }
        ...
        Storage.prototype.addRoot = function (el) {
            ...
            if (el instanceof Group) {//Group的_children也会被递归加入Storage
                el.addChildrenToStorage(this);
            }

            this.addToMap(el);//加入索引map
            this._roots.push(el);//加入列表保存
        };
