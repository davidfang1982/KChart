 ...
 define(
    function (require) {
        ...
        var Painter = function (root, storage) {
            ...
            root.innerHTML = '';//id为main
            this._width = this._getWidth(); // 宽，缓存记录
            this._height = this._getHeight(); // 高，缓存记录

            var domRoot = document.createElement('div');//创建一个main下的子div
            ...
            this._zlevelList = [];//shape设置了zlevel，保存0  1 2 这些zlevel
            ...
            this._bgDom.className = config.elementClassName;//elementClassName: 'zr-element',

            domRoot.appendChild(this._bgDom);
            this._bgDom.onselectstart = returnFalse;//背景层不能选择

            // 高亮
            var hoverLayer = new Layer('_zrender_hover_', this);
            this._layers['hover'] = hoverLayer;
            domRoot.appendChild(hoverLayer.dom);
            hoverLayer.initContext();//设置layer的ctx
        ...
        Painter.prototype.refresh = function (callback, paintAll) {
            var list = this.storage.getShapeList(true);//获取要绘制的shape
            this._paintList(list, paintAll);//在下面定义，具体内容：

            // Paint custum layers
            for (var i = 0; i < this._zlevelList.length; i++) {
                var z = this._zlevelList[i];
                var layer = this._layers[z];//获取zlevel对应的layer
        ...
        Painter.prototype._preProcessLayer = function (layer) {
            layer.unusedCount++;
            layer.updateTransform();//是否需要矩阵转换
        };
        ...
        Painter.prototype._paintList = function (list, paintAll) {
            ...
            for (var i = 0, l = list.length; i < l; i++) {
                ...
                if (currentZLevel !== shape.zlevel) {
                    if (currentLayer) {
                        if (currentLayer.needTransform) {
                            ctx.restore();//canvas.save();和canvas.restore();是两个相互匹配出现的，作用是用来保存画布的状态和取出保存的状态的
                        }
                        ctx.flush && ctx.flush();
                    }

                    currentZLevel = shape.zlevel;
                    currentLayer = this.getLayer(currentZLevel);//获取zlevel的层 若无则创建
                ...
                //若layerdirty 且shape可见
                if ((currentLayer.dirty || paintAll) && !shape.invisible) {
                    if (
                        !shape.onbrush
                        || (shape.onbrush && !shape.onbrush(ctx, false))//刷新前调用onbrush，返回false继续调用
                    ) {
                        if (config.catchBrushException) {
                            try {
                                shape.brush(ctx, false, this.refreshNextFrame);//调用brush绘制图形
                            }
                            catch (error) {
                                log(
                                    error,
        ...
        Painter.prototype._updateLayerStatus = function (list) {
            ...
            for (var i = 0, l = list.length; i < l; i++) {
                ...
                if (layer) {
                    ...
                    layer.dirty = shape.__dirty;//若某个shape的__dirty为true则之后不会设置 layer.dirty 
                }
            }

            // 层中的元素数量有发生变化
            //遍历list若zlevel层的数量和layer的不一样，需要刷新
            this.eachBuildinLayer(function (layer, z) {
        ...
        Painter.prototype.refreshHover = function () {
            this.clearHover();
            var list = this.storage.getHoverShapes(true);//获取hover层的shape
            for (var i = 0, l = list.length; i < l; i++) {
